...
export default function WinterHat4({ children, hatColor, facialHairType, facialHairColor }) {
	const [ path1Id ] = useState(nanoid);
	const [ path2Id ] = useState(nanoid);
	const [ path3Id ] = useState(nanoid);
	const [ path4Id ] = useState(nanoid);
	const [ path5Id ] = useState(nanoid);
	const [ mask1Id ] = useState(nanoid);
	const [ mask2Id ] = useState(nanoid);
	const [ mask3Id ] = useState(nanoid);
	const [ mask4Id ] = useState(nanoid);
	...
	const path4 = `react-path-${path4Id}`;
	const path5 = `react-path-${path5Id}`;
	const mask1 = `react-mask-${mask1Id}`;
	const mask2 = `react-mask-${mask2Id}`;
	const mask3 = `react-mask-${mask3Id}`;
	const mask4 = `react-mask-${mask4Id}`;
	const filter1 = `react-mask-${filter1Id}`;
	...
	return (
		<g id="Top">
			...
			<g id="Top/Accessories/Winter-Hat-4" transform="translate(-1.000000, 0.000000)">
				<g id="hat" strokeWidth="1" fillRule="evenodd" transform="translate(65.000000, 4.000000)">
					...
					<use id="light" fillOpacity="0.300000012" fill="#FFFFFF" xlinkHref={'#' + path3} />
				</g>
				<g id="hat-front">
